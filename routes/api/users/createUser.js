    ...
    if (findUser) {
      ...
      var mailOptions = {
        from:
          ...
          `"What a Duck!" <${process.env.EMAIL_USER}>`,
        to: newUser.email,
        subject: " What a Duck! 🦆 Verifica tu correo electronico✅",
        text: `¡Gracias por registrarte en What a Duck! Bienvenido, ${newUser.name}. Por favor, verifica tu correo electrónico en el siguiente enlace: http://${req.headers.host}/verify-email?token=${newUser.emailToken}`,
        html: `<h2>¡Gracias por registrarte en What a Duck! Bienvenido,  ${newUser.name}</h2>
                ...
                <a href="http://${req.headers.host}/api/users/verify-email?token=${newUser.emailToken}">Verificar Correo</a>`,
      };
